...
MacroSystem.library          = {}
MacroSystem.macroMapsReady   = false
...
GameTab:CreateToggle({
    ...
    CurrentValue = false,
    Flag         = "AutoVoteExtract",
    Info         = "Votes to continue every 5 waves until the target wave, then extracts.",
    Callback     = function(v)
        State.AutoVoteExtract = v
    end,
})

GameTab:CreateSlider({
    Name         = "Extract on Wave",
    Range        = { 5, 200 },
    Increment    = 5,
    CurrentValue = 5,
    Suffix       = " waves",
    Flag         = "AutoVoteExtractWave",
    Info         = "Votes to continue until this wave, then votes to extract.",
    Callback     = function(v)
        State.AutoVoteExtractWave = v
    end,
})
...
local function setupAutoAbility()
    ...
    enemyRemote.OnClientEvent:Connect(function(buf, refs)
        ...
        if refs[6] ~= "boss" then return end
